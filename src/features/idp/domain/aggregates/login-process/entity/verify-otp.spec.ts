...
describe('LoginProcessEntity.verifyOtp', () => {
  describe('из состояния OtpRequested', () => {
    it('возвращает otp_expired если OTP истёк', () => {
      ...
      const result = verifyOtp(state, makeCommand({ now: afterExpiry }));

      expect(isLeft(result)).toBe(false);
      if (!isLeft(result)) {
        expect(result.value.event).toEqual({ type: 'login_process.otp_expired' });
        expect(result.value.state.type).toBe('Errored');
      }
    });

    it('возвращает otp_verify_failed при неверном коде', () => {
      const state = makeOtpRequested();
      const result = verifyOtp(state, makeCommand({ otpCode: WRONG_OTP }));
    ...
    it('возвращает blocked после MAX_OTP_ATTEMPTS неверных попыток', () => {
      ...
      expect(isLeft(result)).toBe(false);
      if (!isLeft(result)) {
        expect(result.value.event.type).toBe('login_process.blocked');
        expect(result.value.state.type).toBe('Blocked');
      }
    });

    it('возвращает new_registration если код верный и пользователь не найден', () => {
      const state = makeOtpRequested();
      const result = verifyOtp(state, makeCommand({ user: undefined }));
      ...
      if (!isLeft(result)) {
        ...
        expect(result.value.state.type).toBe('NewRegistration');
      }
    });
    ...
    it('возвращает completed если код верный и пользователь найден', () => {
      ...
      if (!isLeft(result)) {
        ...
        expect(result.value.state.type).toBe('Success');
      }
    });
  });
  ...
  describe('из состояния Blocked', () => {
    it('возвращает LoginBlockedError если блокировка активна', () => {
      ...
      const result = verifyOtp(makeBlocked(blockedUntil), makeCommand());
    ...
    it('возвращает InvalidOtpError если блокировка истекла', () => {
  ...
  describe('из невалидных состояний', () => {
    it('возвращает InvalidOtpError для null', () => {
      expect(verifyOtp(null, makeCommand())).toEqual(Left(new InvalidOtpError()));
    });
    ...
    it('возвращает InvalidOtpError для Success', () => {
      ...
      expect(verifyOtp(state, makeCommand())).toEqual(Left(new InvalidOtpError()));
    });
  });
});

...
function makeDeps() {
  ...
  categoryDirectory.findByIds.mockResolvedValue([]);
  cityCoordinates.findCoordinates.mockResolvedValue(null);
  return { client, categoryDirectory, cityCoordinates };
}
...
describe('GorseSyncAdapter', () => {
  it('item with coordinates + categories → geo categories with ageGroup and rootCat', async () => {
    ...
    deps.categoryDirectory.findByIds.mockResolvedValue([
      {
        categoryId: CategoryId.raw('cat-1'),
        parentCategoryId: rootCatId,
        name: 'cat',
        iconId: 'icon' as never,
        order: 0,
        status: 'published' as const,
        allowedTypeIds: [],
        ancestorIds: [rootCatId],
        ageGroups: [],
        attributes: [],
    ...
    await adapter.upsertItem(
      makeItem({
        ...
        category: {
          ...
          closureCategoryIds: [CategoryId.raw('cat-1'), rootCatId],
          attributeValues: [],
        },
      }),
    );
  ...
  it('item without categories → only geo, no rootCat cross-product', async () => {
    ...
    expect(deps.categoryDirectory.findByIds).not.toHaveBeenCalled();
  });
  ...
  it('labels are not affected (type:, attr:, age:, h3:)', async () => {
    ...
    await adapter.upsertItem(
      makeItem({
        ...
        category: {
          categoryIds: [CategoryId.raw('cat-1')],
          closureCategoryIds: [CategoryId.raw('cat-1')],

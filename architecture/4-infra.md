...
```ts
export const DiscoverySearchClient = CreateSearchClient('DiscoverySearchClient');

SearchModule.register({
  node: config.get('MEILISEARCH_URL'),
  username: '...',
  password: '...',
  clients: [DiscoverySearchClient],
})
```
...
Хелпер для формирования типизированных HTTP-ошибок:

```ts
throw apiError('requestOtp', { code: 'otp_throttle', retryAfterSec: 30 }, 429);
```
...
OpenAPI-типы для request/response:

...
describe('LoginProcessEntity.register', () => {
  it('возвращает state и event при корректных данных', () => {
    const result = register(makeNewRegistration(), makeCommand());
  ...
  describe('из невалидных состояний', () => {
    it('возвращает RegistractionError для null', () => {
      expect(register(null, makeCommand())).toEqual(Left(new RegistractionError()));
    });
    ...
    it('возвращает RegistractionError для OtpRequested', () => {
      ...
      expect(register(state, makeCommand())).toEqual(Left(new RegistractionError()));
    });

...
@Injectable()
export class AddCloseSubscriptionInteractor {
  public constructor(
  ...
  public async execute(command: {
    ...
    return this.txHost.startTransaction(async (tx) => {
      ...
      const subscriptionId = this.idGenerator.generateBoardCloseSubscriptionId();
      const now = this.clock.now();

      const result = BoardEntity.addCloseSubscription(state, {
